<?php
/**
 * woocommerce_mobapp API Reports Class
 *
...
class WOOAPP_API_Reports extends WOOAPP_API_Resource {
	...
	public function register_routes( $routes ) {
		...
		$routes[ $this->base ] = array(
			array( array( $this, 'get_reports' ),     WOOAPP_API_Server::READABLE ),
		);

		# GET /reports/sales
		$routes[ $this->base . '/sales'] = array(
			array( array( $this, 'get_sales_report' ), WOOAPP_API_Server::READABLE ),
		);

		# GET /reports/sales/top_sellers
		$routes[ $this->base . '/sales/top_sellers' ] = array(
			array( array( $this, 'get_top_sellers_report' ), WOOAPP_API_Server::READABLE ),
		);
	...
	public function get_sales_report( $fields = null, $filter = array() ) {
		...
		return array( 'sales' => apply_filters( 'woocommerce_mobapp_api_report_response', $sales_data, $this->report, $fields, $this->server ) );
	}
	...
	public function get_top_sellers_report( $fields = null, $filter = array() ) {
		...
		return array( 'top_sellers' => apply_filters( 'woocommerce_mobapp_api_report_response', $top_sellers_data, $this->report, $fields, $this->server ) );
	}
	...
	private function setup_report( $filter ) {
		...
		if ( empty( $filter['period'] ) ) {
			...
			if ( ! empty( $filter['date_min'] ) || ! empty( $filter['date_max'] ) ) {
				...
				// overwrite _GET to make use of WOOAPP_Admin_Report::calculate_current_range() for custom date ranges
			...
			// TODO: change WOOAPP_Admin_Report class to use "week" instead, as it's more consistent with other periods
	...
	protected function validate_request( $id = null, $type = null, $context = null ) {
		...
		if ( ! current_user_can( 'view_woocommerce_mobapp_reports' ) ) {

			return new WOOAPP_API_Error( 'woocommerce_mobapp_api_user_cannot_read_report', __( 'You do not have permission to read this report', 'woocommerce_mobapp' ), array( 'status' => 401 ) );
