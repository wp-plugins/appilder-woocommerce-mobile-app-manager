<?php
/**
 * woocommerce_mobapp API Coupons Class
 *
...
class WOOAPP_API_Coupons extends WOOAPP_API_Resource {
	...
	public function register_routes( $routes ) {
		...
		$routes[ $this->base ] = array(
			array( array( $this, 'get_coupons' ),     WOOAPP_API_Server::READABLE ),
		);

		# GET /coupons/count
		$routes[ $this->base . '/count'] = array(
			array( array( $this, 'get_coupons_count' ), WOOAPP_API_Server::READABLE ),
		);

		# GET /coupons/<id>
		$routes[ $this->base . '/(?P<id>\d+)' ] = array(
			array( array( $this, 'get_coupon' ),  WOOAPP_API_Server::READABLE ),
		);
		...
		$routes[ $this->base . '/code/(?P<code>\w[\w\s\-]*)' ] = array(
			array( array( $this, 'get_coupon_by_code' ), WOOAPP_API_Server::READABLE ),
		);
	...
	public function get_coupon( $id, $fields = null ) {
		...
		if ( is_null( $code ) )
			return new WOOAPP_API_Error( 'woocommerce_mobapp_api_invalid_coupon_id', __( 'Invalid coupon ID', 'woocommerce_mobapp' ), array( 'status' => 404 ) );
		...
		return array( 'coupon' => apply_filters( 'woocommerce_mobapp_api_coupon_response', $coupon_data, $coupon, $fields, $this->server ) );
	}
	...
	public function get_coupons_count( $filter = array() ) {
		...
		if ( ! current_user_can( 'read_private_shop_coupons' ) )
			return new WOOAPP_API_Error( 'woocommerce_mobapp_api_user_cannot_read_coupons_count', __( 'You do not have permission to read the coupons count', 'woocommerce_mobapp' ), array( 'status' => 401 ) );
	...
	public function get_coupon_by_code( $code, $fields = null ) {
		...
		if ( is_null( $id ) )
			return new WOOAPP_API_Error( 'woocommerce_mobapp_api_invalid_coupon_code', __( 'Invalid coupon code', 'woocommerce_mobapp' ), array( 'status' => 404 ) );
