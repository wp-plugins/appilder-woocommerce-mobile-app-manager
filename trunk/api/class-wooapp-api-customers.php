...
class WOOAPP_API_Customers extends WOOAPP_API_Resource {
	...
	public function __construct( WOOAPP_API_Server $server ) {
		...
		add_filter( 'woocommerce_mobapp_api_order_response', array( $this, 'add_customer_data' ), 10, 2 );
	...
	public function register_routes( $routes ) {
		...
		$routes[ $this->base ] = array(
			array( array( $this, 'get_customers' ),     WOOAPP_API_SERVER::READABLE ),
		);

		# GET /customers/count
		$routes[ $this->base . '/count'] = array(
			array( array( $this, 'get_customers_count' ), WOOAPP_API_SERVER::READABLE ),
		);

		# GET /customers/<id>
		$routes[ $this->base . '/(?P<id>\d+)' ] = array(
			array( array( $this, 'get_customer' ),  WOOAPP_API_SERVER::READABLE ),
		);

		# GET /customers/<id>/orders
		$routes[ $this->base . '/(?P<id>\d+)/orders' ] = array(
			array( array( $this, 'get_customer_orders' ), WOOAPP_API_SERVER::READABLE ),
		);
	...
	public function get_customer( $id, $fields = null ) {
        ...
        if(!(is_user_logged_in() && get_current_user_id() == $id))
            return new WOOAPP_API_Error( 'woocommerce_mobapp_api_invalid_customer', __( 'Invalid customer', 'woocommerce_mobapp' ), array( 'status' => 404 ) );
		...
		return array( 'customer' => apply_filters( 'woocommerce_mobapp_api_customer_response', $customer_data, $customer, $fields, $this->server ) );
	}
	...
	public function get_customers_count( $filter = array() ) {
		...
		if ( ! current_user_can( 'list_users' ) )
			return new WOOAPP_API_Error( 'woocommerce_mobapp_api_user_cannot_read_customers_count', __( 'You do not have permission to read the customers count', 'woocommerce_mobapp' ), array( 'status' => 401 ) );
	 ...
	 * Create a customer
	 *
	 * @TODO implement in 2.2 with woocommerce_mobapp_create_new_customer()
	 * @param array $data
	 * @return array
	 */
	public function create_customer( $data ) {
		...
		if ( ! current_user_can( 'create_users' ) )
			return new WOOAPP_API_Error( 'woocommerce_mobapp_api_user_cannot_create_customer', __( 'You do not have permission to create this customer', 'woocommerce_mobapp' ), array( 'status' => 401 ) );
	...
	public function get_customer_orders( $id, $fields = null ) {
		...
		$id = $this->is_readable( $id, 'customer', 'read' );
		...
		foreach ( $order_ids as $order_id ) {
			$orders[] = current( getapi()->WOOAPP_API_Orders->get_order( $order_id, $fields ) );
		}

		return array( 'orders' => apply_filters( 'woocommerce_mobapp_api_customer_orders_response', $orders, $id, $fields, $order_ids, $this->server ) );
	}
	 ...
	 *
	 * @since 2.1
	 * @see WOOAPP_API_Resource::validate_request()
	 ...
	 * @return int|WOOAPP_API_Error valid user ID or WOOAPP_API_Error if any of the checks fails
	 */
	protected function validate_request( $id, $type, $context ) {
		...
		// validate ID
		if ( empty( $id ) )
			return new WOOAPP_API_Error( 'woocommerce_mobapp_api_invalid_customer_id', __( 'Invalid customer ID', 'woocommerce_mobapp' ), array( 'status' => 404 ) );
		...
		switch ( $context ) {
			...
			case 'read':
				if ( ! current_user_can( 'list_users' ) )
					return new WOOAPP_API_Error( 'woocommerce_mobapp_api_user_cannot_read_customer', __( 'You do not have permission to read this customer', 'woocommerce_mobapp' ), array( 'status' => 401 ) );
				break;

			case 'edit':
				if ( ! current_user_can( 'edit_users' ) )
					return new WOOAPP_API_Error( 'woocommerce_mobapp_api_user_cannot_edit_customer', __( 'You do not have permission to edit this customer', 'woocommerce_mobapp' ), array( 'status' => 401 ) );
				break;

			case 'delete':
				if ( ! current_user_can( 'delete_users' ) )
					return new WOOAPP_API_Error( 'woocommerce_mobapp_api_user_cannot_delete_customer', __( 'You do not have permission to delete this customer', 'woocommerce_mobapp' ), array( 'status' => 401 ) );
				break;
		}
