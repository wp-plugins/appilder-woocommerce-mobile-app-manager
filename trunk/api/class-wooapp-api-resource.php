<?php
/**
 * woocommerce_mobapp API Resource class
 *
...
class WOOAPP_API_Resource {

	/** @var WOOAPP_API_Server the API server */
	protected $server;
	...
	public function __construct( WOOAPP_API_Server $server ) {
		...
		add_filter( 'woocommerce_mobapp_api_endpoints', array( $this, 'register_routes' ) );
		...
		foreach ( array( 'order', 'coupon', 'customer', 'product', 'report' ) as $resource ) {
			...
			add_filter( "woocommerce_mobapp_api_{$resource}_response", array( $this, 'maybe_add_meta' ), 15, 2 );
			add_filter( "woocommerce_mobapp_api_{$resource}_response", array( $this, 'filter_response_fields' ), 20, 3 );
		}
	}
	 ...
	 * @return int|WOOAPP_API_Error valid post ID or WOOAPP_API_Error if any of the checks fails
	 */
	protected function validate_request( $id, $type, $context ) {
		...
		// validate ID
		if ( empty( $id ) )
			return new WOOAPP_API_Error( "woocommerce_mobapp_api_invalid_{$resource_name}_id", sprintf( __( 'Invalid %s ID', 'woocommerce_mobapp' ), $type ), array( 'status' => 404 ) );
		...
		if ( 'customer' !== $type ) {
			...
			// validate post type
			if ( $type !== $post_type )
				return new WOOAPP_API_Error( "woocommerce_mobapp_api_invalid_{$resource_name}", sprintf( __( 'Invalid %s', 'woocommerce_mobapp' ), $resource_name ), array( 'status' => 404 ) );
			...
			switch ( $context ) {
				...
				case 'read':
                    if ( ! $this->is_readable( $post ) )
						return new WOOAPP_API_Error( "woocommerce_mobapp_api_user_cannot_read_{$resource_name}", sprintf( __( 'You do not have permission to read this %s', 'woocommerce_mobapp' ), $resource_name ), array( 'status' => 401 ) );
					break;

				case 'edit':
					if ( ! $this->is_editable( $post ) )
						return new WOOAPP_API_Error( "woocommerce_mobapp_api_user_cannot_edit_{$resource_name}", sprintf( __( 'You do not have permission to edit this %s', 'woocommerce_mobapp' ), $resource_name ), array( 'status' => 401 ) );
					break;

				case 'delete':
					if ( ! $this->is_deletable( $post ) )
						return new WOOAPP_API_Error( "woocommerce_mobapp_api_user_cannot_delete_{$resource_name}", sprintf( __( 'You do not have permission to delete this %s', 'woocommerce_mobapp' ), $resource_name ), array( 'status' => 401 ) );
					break;
			}
	...
	protected function merge_query_args( $base_args, $request_args ) {
		...
		if ( ! empty( $request_args['cat'] ) )
			$args['cat'] = $request_args['cat'];

		if ( ! empty( $request_args['category_name'] ) )
			$args['category_name'] = $request_args['category_name'];

		if ( ! empty( $request_args['category__in'] ) )
			$args['category__in'] = $request_args['category__in'];

        $order = isset($request_args['order'])?$request_args['order']:'';
        $orderby = isset($request_args['orderby'])?$request_args['orderby']:'';
        $sort_args = WC()->query->get_catalog_ordering_args($orderby,$order);
        ...
        */
		...
		if ( ! empty( $request_args['product_cat'] ) ){
            $request_args['product_cat'] =  explode(',',$request_args['product_cat']);
            $args['tax_query'][] =
                array(
                    'taxonomy' => 'product_cat',
                    'field' => 'slug',
                    'terms' => $request_args['product_cat']
                );
        }

		if ( ! empty( $request_args['product_cat_id'] ) ){
            $request_args['product_cat_id'] =  explode(',',$request_args['product_cat_id']);
            $args['tax_query'][] =
                array(
                    'taxonomy' => 'product_cat',
                    'field' => 'id',
                    'terms' => $request_args['product_cat_id']
                );
        }

        if ( ! empty( $request_args['attributes_filter'])){
            parse_str($request_args['attributes_filter'],$taxs);
            foreach($taxs as $tax=>$slug_value) {
                // $slug_value = explode(",",$tax);
        ...
        if (isset($request_args['price_filter_min'])){
                $args['meta_query'][] = array(
                    'key' => '_price', //_price
                    'value' => $request_args['price_filter_min'],
                    'compare' => '>=',
                    'type' => 'numeric'
                );
        }

        if (isset($request_args['price_filter_max'])){
            $args['meta_query'][] = array(
                'key' => '_price', //_price
                'value' => $request_args['price_filter_max'],
                'compare' => '<=',
                'type' => 'numeric'
            );
        }
        ...
        if(isset($args['tax_query']) && count($args['tax_query'])>1) $args['meta_query']['relation'] = "AND";
        // resource page
	 ...
	 * @param object $resource the resource object (e.g WOOAPP_Order)
	 * @return mixed
	 */
	public function maybe_add_meta( $data, $resource ) {
		...
		if ( isset( $this->server->params['GET']['filter']['meta'] ) && 'true' === $this->server->params['GET']['filter']['meta'] && is_object( $resource ) ) {
			...
			} elseif ( is_a( $resource, 'WOOAPP_Product_Variation' ) ) {
	 ...
	 * @param object $resource the object that provided the response data, e.g. WOOAPP_Coupon or WOOAPP_Order
	...
	protected function delete( $id, $type, $force = false ) {
		...
		if ( 'customer' === $type ) {
			...
			if ( $result )
				return array( 'message' => __( 'Permanently deleted customer', 'woocommerce_mobapp' ) );
			else
				return new WOOAPP_API_Error( 'woocommerce_mobapp_api_cannot_delete_customer', __( 'The customer cannot be deleted', 'woocommerce_mobapp' ), array( 'status' => 500 ) );
			...
			if ( ! $result )
				return new WOOAPP_API_Error( "woocommerce_mobapp_api_cannot_delete_{$resource_name}", sprintf( __( 'This %s cannot be deleted', 'woocommerce_mobapp' ), $resource_name ), array( 'status' => 500 ) );

			if ( $force ) {
				return array( 'message' => sprintf( __( 'Permanently deleted %s', 'woocommerce_mobapp' ), $resource_name ) );

			} else {

				$this->server->send_status( '202' );

				return array( 'message' => sprintf( __( 'Deleted %s', 'woocommerce_mobapp' ), $resource_name ) );
			}
		}
	}
	 ...
	 *
	 * @since 2.1
	 * @see WOOAPP_API_Resource::check_permission()
