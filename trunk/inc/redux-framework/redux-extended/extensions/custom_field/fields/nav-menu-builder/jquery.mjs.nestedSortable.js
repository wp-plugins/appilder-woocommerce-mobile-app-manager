...
jQuery(document).ready(function(){
  window.catlistmenuf =  function(){
      window.catlistmenu = jQuery('.nav_menu_items_space > ol').nestedSortable({
        isTree: true,
    ...
    window.save_update = function(){
        var a = jQuery('.nav_menu_items_space > ol').nestedSortable('toArray');
        window.catvalues=[];
        ...
        jQuery.each(a,function( index, element ) {
            if(element.item_id != "" &&  element.item_id!=null) {
                ...
                var  list = jQuery('li[id="list_'+temp_id+'"]');
                if(typeof list != "undefined"){
                      var li_id = list.attr("id").match(/list_(.*)/)[1];
                      jQuery(list).find(".parentfield").val(element.parent_id);
                }
            }
        });
    }
