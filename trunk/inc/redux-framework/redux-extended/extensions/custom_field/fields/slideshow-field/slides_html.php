...
// Don't duplicate me!
if (!class_exists('ReduxFramework_slides_html')) {
    /**
     * Main ReduxFramework_slides_html class
     *
     * @since       1.0.0
     */
    class ReduxFramework_slides_html extends widget_handler{
         ...
         * @param string $value
         * @param $parent
         * @return \ReduxFramework_slides_html
         */
        function __construct( $field = array(), $value ='', $parent ) {
          parent::__construct($field,$value,$parent);
            ...
            include_once(ReduxFramework::$_dir. "inc/fields/editor/field_editor.php");
         ...
         $filed_name = $this->field['name'];
             ...
             if (isset($this->value['slides']) && is_array($this->value['slides']) && !$hidden) {
                ...
                foreach ($slides as $slide) {
                    ...
                    echo '<li>';
                    $args = array("id"=>$this->field['id']."-content-".$x,"name"=>$this->field['name'] . '[' . $x . '][content]');
                    $args = wp_parse_args($args,$this->editor_args);
                    $slide['content'] = isset($slide['content'])?$slide['content']:'';
                    $editor = new ReduxFramework_editor($args,$slide['content'],$this->parent);
                    $editor->render();
                    echo '</li>';
            ...
            if ($x == 0) {
                ...
                $args = array("id"=>$this->field['id']."-content-".$x,"name"=>$this->field['name'] . '[' . $x . '][content]');
                $args = wp_parse_args($args,$this->editor_args);
                $editor = new ReduxFramework_editor($args,'',$this->parent);
                $editor->render();
                echo '</li>';
