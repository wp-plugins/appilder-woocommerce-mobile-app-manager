...
// Don't duplicate me!
if (!class_exists('ReduxFramework_slides_search')) {
    /**
     * Main ReduxFramework_slides_product_scroller class
     *
     * @since       1.0.0
     */
    class ReduxFramework_slides_search extends widget_handler{
         ...
         * @param string $value
         * @param $parent
         * @return \ReduxFramework_slides_search
         */
        ...
        public function blank_slider(){
            $this->widget_field(widget_handler::$category_selector,true,array("multi"=>true,"sortable"=>false,"width"=>"80",'title'=>"Products","class"=>"select2"));
        }
        ...
        public function render($hidden = false) {
            ...
            if (isset($this->value['slides']) && is_array($this->value['slides']) && !$hidden) {
                ...
                foreach ($slides as $slide) {
                    ...
                    $action_value = is_array($slide['click_action_value'])?implode(",",$slide['click_action_value']):'';
                    echo '<li style="display:none" class="widget-search-action widget-action-value-field" data-name="' . $this->field['name'] . '[' . $x . '][click_action_value][]" data-value="'.$action_value.'" data-id="' . $this->field['name'] . '-' . $x . '-click_action_value"></li>';
                    //                echo '<li>';
  //                  $this->widget_field(widget_handler::$category_selector,false,array("multi"=>true,"sortable"=>false,"width"=>"80","name"=>$this->field['name'] . '[' . $x . '][products]',"id"=>$this->field['id'] . '-' . $x . '-click_action_value','title'=>"Products","class"=>"select2"),$slide['products']);
      //              echo '</li>';
